...
export async function updateCodeIDEConfigMapJson() {
    ...
    const previousCodeVersion = await getIDEVersionOfImage("ghcr.io/nxpkg/build/" + ideConfigmapJson.ideOptions.options.code.image.replace("{{.Repository}}/", ""));
    const installationCodeVersion = await getIDEVersionOfImage("ghcr.io/nxpkg/build/" + newJson.ideOptions.options.code.image.replace("{{.Repository}}/", ""));
